...
maskk = cv2.bitwise_or(mask,mask3)
cv2.imshow("screen",maskk)
kernel = np.ones((5,5),np.uint8)
mask = cv2.morphologyEx(maskk,cv2.MORPH_CLOSE,kernel)
rainbow[maskk>0]=[81,62,39]
cv2.imshow("screen",rainbow)
cv2.waitKey(0)
